...
import { hytaleJar, type HytaleJar } from "./HytaleApi";
...
export function getLeftDiff(): DiffSide {
    // For now, diff functionality is disabled until we implement version fetching
    if (!leftDiff) {
        leftDiff = {} as DiffSide;
        leftDiff.selectedVersion = new BehaviorSubject<string | null>(null);
        leftDiff.jar = hytaleJar; // TODO: implement version selection for diff
...
export function getRightDiff(): DiffSide {
    if (!rightDiff) {
        rightDiff = {
            selectedVersion: new BehaviorSubject<string | null>("local"),
            jar: hytaleJar,
            entries: hytaleJar.pipe(
                switchMap(jar => from(getEntriesWithCRC(jar)))
            ),
...
async function getEntriesWithCRC(jar: HytaleJar): Promise<Map<string, EntryInfo>> {
